...
class _MyAppState extends State<MyApp> {
  ...
  void removeProcessedData(
    ...
    setState(() {
      _processedData.removeAt(
        index,
      );
    });
  }

  void callbackToSetParameter(
    String inputMaximumObject,
    String inputConfidenceThreshold,
  ) {
    setState(() {
  ...
  _getImageAndDectec(
    ...
    var results = await detectObjectFromImage(
      image,
      10,
      double.parse(
        confidenceThreshold,
  ...
  Widget build(BuildContext context) {
    ...
    Widget _buildGridView() {
      return _processedData.length != 0
          ? GridView.count(
              mainAxisSpacing: 4.0,
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          actions: [
            _buildParameterSettingMenuButton(),
          ],
          bottom: TabBar(
          ...
          GestureDetector(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => DetectedDetails(
                    data: widget.data,
                  ),
                ),
              );
...
class ParameterDialogState extends State<ParameterDialog> {
  ...
  String _validateConfidenceThreshold(String value) {
    if (value.isEmpty) return 'The parameter is required.';
      ...
      actions: <Widget>[
        FlatButton(
          ...
          onPressed: () {
            ...
            setState(
              () {
                widget.callbackToSetParameter(
                  currentMaximumObject,
