...
class DetectedDetailsState extends State<DetectedDetails> {
  Map _buildPivotData() {
    ...
    for (String label in setLabels) {
      int first = sortedLabelList.indexOf(
        label,
      );
      int last = sortedLabelList.lastIndexOf(
        label,
      );
      objectNumbers.add(
        last - first + 1,
  ...
  Widget _buildTable() {
    return PaginatedDataTable(
      ...
      rowsPerPage: widget.data["recognitions"]["id"].length > 5
          ? 5
          : widget.data["recognitions"]["id"].length,
      columns: <DataColumn>[
        DataColumn(
          label: const Text(
            'ID',
      ...
      source: DetailObjectDataSource(
        widget.data["recognitions"],
      ),
    );
  }
  ...
  Widget _builPivotTable() {
    ...
    return PaginatedDataTable(
      ...
      rowsPerPage:
          sumaryData["label"].length > 5 ? 5 : sumaryData["label"].length,
      columns: <DataColumn>[
        DataColumn(
          label: const Text(
            'ID',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: widget.data != null
          ? widget.data["recognitions"] != null &&
                  widget.data["recognitions"]["label"].length != 0
              ? ListView(
